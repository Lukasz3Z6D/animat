...
void elipse(int x, int y, int diamX,int diamY,int colorRGB= RGB(0, 255, 0))
{
	...
	PEN=getPen(PS_SOLID, min(diamX, diamY)-2, colorRGB);
	SelectObject(hDC, PEN);

	BGN.X = (short)x + movx - diamX / 2;
	BGN.Y = (short)y + mov - diamY / 2;
	POS.X = (short)x + movx + diamX / 2;
	POS.Y = (short)y + mov + diamY / 2;
	PlotCirc(hDC, PEN, BGN, POS);
}
